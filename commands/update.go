...
func updateBaseCore(cmd *mamba.Command, args []string) {
	cmd.PrintInfo("Updating Base Core...")
	err := updateCore(cmd)
	if err != nil {
		cmd.PrintError("Error updating Base Core: " + err.Error())
		return
	}
	cmd.PrintSuccess("Base Core updated successfully")
}

func updateCore(cmd *mamba.Command) error {
	...
	// Download the tagged archive with spinner
	var tmpZip *os.File
	downloadErr := spinner.WithSpinner("Downloading core archive...", func() error {
	...
	cmd.PrintSuccess("Core directory updated successfully")
	return nil
}
