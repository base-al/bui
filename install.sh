...
mkdir -p "$BIN_DIR" 2>/dev/null || {
    ...
    echo "curl -sSL https://raw.githubusercontent.com/base-al/admin-cli/main/install.sh | sudo bash"
    exit 1
}
...
API_RESPONSE=$(curl -s https://api.github.com/repos/base-al/admin-cli/releases/latest)
if [ $? -ne 0 ]; then
...
if [ -z "$LATEST_RELEASE" ]; then
    ...
    echo "Repository: https://github.com/base-al/admin-cli"
    exit 1
fi
